...
struct BackendData {
	...
	bool buffer_dimensions_dirty = true;
...
bool Backend::Initialize(const char* window_name, int width, int height, bool allow_resize)
{
	...
	if (!CreateDeviceD3D(window_handle, data->device_resources))
	{
...
void Backend::BeginFrame()
{
	...
	if (data->buffer_dimensions_dirty)
	{
		data->buffer_dimensions_dirty = false;
		CleanupRenderTarget(data->device_resources);
		if (data->window_dimensions != Rml::Vector2i{0, 0})
		{
			data->device_resources.pSwapChain->ResizeBuffers(0, data->window_dimensions.x, data->window_dimensions.y, DXGI_FORMAT_UNKNOWN, 0);
...
static LRESULT CALLBACK WindowProcedureHandler(HWND window_handle, UINT message, WPARAM w_param, LPARAM l_param)
{
	...
	case WM_SIZE:
	{
		...
		data->window_dimensions = Rml::Math::Max(Rml::Vector2i{1, 1}, {width, height});
		data->context_dimensions_dirty = true;
		data->buffer_dimensions_dirty = true;
