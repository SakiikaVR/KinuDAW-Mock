...
**v0.3.2は実処理を備えた開発版です。** 基本的な制作機能を動作確認していますが、市販DAWと同等の機能・全プラグインの全プリセットでの互換性を保証するものではありません。未対応機能と検証範囲を下記に記載しています。

## 起動

1. [Windows x64開発版](https://github.com/SakiikaVR/KinuDAW-Mock/releases/tag/v0.3.2)のZIPを全体展開します。
...
- テスト用VST3の257／513サンプル遅延、直列＋マスターの771サンプル整列、書き出し先頭補正、CCのサンプル位置・16チャンネル、内部MIDI出力、循環拒否、パラメーター自動化を検証。Vitalでモックと同じタブ、検索、スライダー、Automation作成・保存、MIDI ON／PORT／CH、ピン留めを実操作で確認。
...
ミキサーのラックは各トラック／マスターの実VST3を表示し、名前クリックで編集、ON/OFFでバイパス、Xで削除できます。VST3追加ではエフェクトだけを選べます。各トラックに追加エフェクト3個、マスターにも3個を接続できます。バイパス状態はプロジェクトに保存します。

プラグインの申告遅延を直列チェーンで合計し、短いトラックを最長のトラックに合わせます。マスターの遅延も画面の再生位置に反映し、WAV書き出しでは先頭の遅延分を取り除いて元の長さを保ちます。遅延変更通知にも対応します。総補正上限は48 kHzで96,000サンプル（2秒）です。上限を超えたプラグインは再生時に自動バイパスし、書き出しはエラーにします。申告されない遅延、外部機器、入力録音位置の補正は対象外です。動的な遅延／バイパス変更時は補正バッファをリセットするため短い途切れが生じます。補正によってライブ演奏の出力待ち時間も増えます。

WinMM入力の16 MIDIチャンネル、CC（サステインを含む）、14-bitピッチベンド、チャンネル／ポリプレッシャーをVST3に渡します。CC・ベンド・チャンネルプレッシャーはIMidiMappingでプラグインが申告したパラメーターへ変換します。割り当てのないCCはプラグインに作用しません。録音したノートのチャンネルと制御イベントを保存・再生し、途中からの再生では直前のCC／ベンド／チャンネルプレッシャーを復元します。制御イベントは1クリップ256件までです。内蔵簡易音源もサステインとベンド（±2半音）に対応します。外部MIDIハードウェアでの実演奏は未検証です。

VST3ウィンドウはモックと同じGUI／MIDI／パラメーターのタブと最前面固定ボタンを備えます。GUIには実際のプラグイン画面を表示します。パラメーターは検索と0..1のスライダーで編集でき、Automationの選択で既存カーブへ接続、＋で専用トラックを作成できます。複数パラメーターを同じカーブに接続でき、最大128接続、8サンプル間隔で送信します。カーブの−48..6の縦軸を正規化0..1に対応させます。読み取り専用パラメーターは操作できません。設定とカーブをプロジェクトへ保存します。

MIDIタブのINPUT／OUTPUTをONにし、同じPORT（00..15）とCH（01..16）を指定すると内部接続します。VST3が出力した対応MIDIイベントを次の処理ブロックで受信プラグインへ送信します。循環接続は停止して表示します。外部WinMM MIDI入力は起動時に自動接続します。MIDIを出力しないプラグインからは内部イベントを転送できません。

補正／CCの実装に使用した公式仕様: [IMidiMapping](https://steinbergmedia.github.io/vst3_doc/vstinterfaces/classSteinberg_1_1Vst_1_1IMidiMapping.html)、[遅延変更](https://steinbergmedia.github.io/vst3_dev_portal/pages/FAQ/Processing.html)。

## 未対応・制約

MIDIプログラムチェンジ、MIDIファイル入出力、サンプル精度の録音位置補正、複数入力機器／個別入力ルーティング、ASIO、タイムストレッチ、テンポマップ、トラックの削除、バス／センドは未実装です。VST3は主ステレオバスを使用し、サイドチェインと複数出力ルーティングは未対応です。再初期化を要求する一部プラグイン操作はワーカーの再読み込みが必要です。
